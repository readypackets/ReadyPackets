...
async function main(): Promise<void> {
  ...
  if (whitelist?.settingValue) {
    try {
      const existing = JSON.parse(whitelist.settingValue);
      ...
      throw new Error("Configured account login whitelist is not valid JSON; migration aborted before verification.");
    }
  }
