...
type Stage = { key: string; label: string; order: number; capabilities: StageCapability[]; actions: StageActions; submissionNotice?: string; uploadLimits?: UploadLimits };
...
function normalizeUploadLimits(value: unknown): UploadLimits | undefined {
  ...
  const raw = value as Record<string, unknown>;
  const number = (key: string) => typeof raw[key] === "number" && Number.isInteger(raw[key]) && raw[key] > 0 ? Math.min(key.includes("Size") ? 100 : 50, raw[key] as number) : undefined;
  const limits = { documentMaxFiles: number("documentMaxFiles"), documentMaxSizeMb: number("documentMaxSizeMb"), audioMaxFiles: number("audioMaxFiles"), audioMaxSizeMb: number("audioMaxSizeMb") };
  return Object.values(limits).some((value) => value !== undefined) ? limits : undefined;
}
function normalizeStages(value: unknown[]): Stage[] {
  if (!Array.isArray(value)) return [];
  return value
    .filter((item): item is { key?: unknown; label?: unknown; order?: unknown; capabilities?: unknown; actions?: unknown; submissionNotice?: unknown; uploadLimits?: unknown } => Boolean(item) && typeof item === "object")
...
function cloneStages(stages: Stage[]): Stage[] {
  return stages.map((stage, index) => ({ ...stage, order: index + 1, capabilities: [...stage.capabilities], actions: { ...stage.actions, adminAlert: stage.actions.adminAlert ? { ...stage.actions.adminAlert } : undefined }, uploadLimits: stage.uploadLimits ? { ...stage.uploadLimits } : undefined }));
}
          ...
          <div className="space-y-3">{stages.map((stage, index) => <div key={`${stage.key}-${index}`} draggable onDragStart={() => setDraggedStage(index)} onDragOver={(event) => event.preventDefault()} onDrop={() => { if (draggedStage !== null) moveStage(draggedStage, index); setDraggedStage(null); }} className={`rounded-xl border bg-white p-5 shadow-sm ${draggedStage === index ? "border-teal ring-2 ring-teal/20" : "border-line"}`}>
            <div className="flex gap-4"><div className="flex flex-col items-center text-muted"><GripVertical className="mt-1 size-5 cursor-grab" /><span className="mt-2 rounded-full bg-teal px-2 py-0.5 text-xs font-bold text-white">{index + 1}</span>{index < stages.length - 1 ? <ArrowDown className="mt-2 size-4 text-teal" /> : null}</div><div className="min-w-0 flex-1 space-y-4">
              ...
              <div><p className="text-xs font-semibold uppercase tracking-wide text-muted">Customer actions enabled in this phase</p><div className="mt-2 grid gap-2 sm:grid-cols-2 xl:grid-cols-4">{CAPABILITIES.map((capability) => { const Icon = capability.icon; return <label key={capability.key} className={`flex cursor-pointer gap-2 rounded-lg border p-2.5 text-sm ${stage.capabilities.includes(capability.key) ? "border-teal/50 bg-teal/5" : "border-line"}`}><Checkbox checked={stage.capabilities.includes(capability.key)} onChange={() => toggleCapability(index, capability.key)} label="" /><span><span className="flex items-center gap-1.5 font-medium text-ink"><Icon className="size-3.5 text-teal" />{capability.label}</span><span className="mt-0.5 block text-xs text-muted">{capability.description}</span></span></label>; })}</div></div>
                            <div className="rounded-lg border border-teal/20 bg-teal/5 p-3"><p className="text-sm font-semibold text-ink">Upload governance</p><p className="mt-1 text-xs text-muted">Optional limits apply to every customer and staff upload in this phase. Leave a field blank to use the platform limit.</p><div className="mt-3 grid gap-3 sm:grid-cols-2 xl:grid-cols-4"><Input label="Document count" type="number" min="1" max="50" placeholder="Platform" value={stage.uploadLimits?.documentMaxFiles ?? ""} onChange={(event) => updateStage(index, { uploadLimits: { ...stage.uploadLimits, documentMaxFiles: event.target.value ? Math.max(1, Math.min(50, Number(event.target.value))) : undefined } })} /><Input label="Document MB each" type="number" min="1" max="100" placeholder="Platform" value={stage.uploadLimits?.documentMaxSizeMb ?? ""} onChange={(event) => updateStage(index, { uploadLimits: { ...stage.uploadLimits, documentMaxSizeMb: event.target.value ? Math.max(1, Math.min(100, Number(event.target.value))) : undefined } })} /><Input label="Audio count" type="number" min="1" max="50" placeholder="Platform" value={stage.uploadLimits?.audioMaxFiles ?? ""} onChange={(event) => updateStage(index, { uploadLimits: { ...stage.uploadLimits, audioMaxFiles: event.target.value ? Math.max(1, Math.min(50, Number(event.target.value))) : undefined } })} /><Input label="Audio MB each" type="number" min="1" max="100" placeholder="Platform" value={stage.uploadLimits?.audioMaxSizeMb ?? ""} onChange={(event) => updateStage(index, { uploadLimits: { ...stage.uploadLimits, audioMaxSizeMb: event.target.value ? Math.max(1, Math.min(100, Number(event.target.value))) : undefined } })} /></div></div>
