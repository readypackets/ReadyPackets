...
type Stage = { key: string; label: string; order: number; capabilities: StageCapability[]; actions: StageActions; adminTasks?: AdminTask[]; customerAcknowledgement?: "required" | "optional" | "none"; submissionNotice?: string; uploadLimits?: UploadLimits; sharePointDestination?: string };
...
function normalizeStages(value: unknown[]): Stage[] {
  ...
  return value
    .filter((item): item is { key?: unknown; label?: unknown; order?: unknown; capabilities?: unknown; actions?: unknown; adminTasks?: unknown; customerAcknowledgement?: unknown; submissionNotice?: unknown; uploadLimits?: unknown; sharePointDestination?: unknown } => Boolean(item) && typeof item === "object")
...
function cloneStages(stages: Stage[]): Stage[] {
  return stages.map((stage, index) => ({ ...stage, order: index + 1, capabilities: [...stage.capabilities], actions: { ...stage.actions, adminAlert: stage.actions.adminAlert ? { ...stage.actions.adminAlert } : undefined }, uploadLimits: stage.uploadLimits ? { ...stage.uploadLimits } : undefined, sharePointDestination: stage.sharePointDestination }));
}
          ...
          <div className="space-y-3">{stages.map((stage, index) => <div key={`stage-editor-${stage.order}`} draggable onDragStart={() => setDraggedStage(index)} onDragOver={(event) => event.preventDefault()} onDrop={() => { if (draggedStage !== null) moveStage(draggedStage, index); setDraggedStage(null); }} className={`rounded-xl border bg-white p-5 shadow-sm ${draggedStage === index ? "border-teal ring-2 ring-teal/20" : "border-line"}`}>
            <div className="flex gap-4"><div className="flex flex-col items-center text-muted"><GripVertical className="mt-1 size-5 cursor-grab" /><span className="mt-2 rounded-full bg-teal px-2 py-0.5 text-xs font-bold text-white">{index + 1}</span>{index < stages.length - 1 ? <ArrowDown className="mt-2 size-4 text-teal" /> : null}</div><div className="min-w-0 flex-1 space-y-4">
              ...
              <div><p className="text-xs font-semibold uppercase tracking-wide text-muted">Customer actions enabled in this phase</p><div className="mt-2 grid gap-2 sm:grid-cols-2 xl:grid-cols-4">{CAPABILITIES.map((capability) => { const Icon = capability.icon; return <label key={capability.key} className={`flex cursor-pointer gap-2 rounded-lg border p-2.5 text-sm ${stage.capabilities.includes(capability.key) ? "border-teal/50 bg-teal/5" : "border-line"}`}><Checkbox checked={stage.capabilities.includes(capability.key)} onChange={() => toggleCapability(index, capability.key)} label="" /><span><span className="flex items-center gap-1.5 font-medium text-ink"><Icon className="size-3.5 text-teal" />{capability.label}</span><span className="mt-0.5 block text-xs text-muted">{capability.description}</span></span></label>; })}</div></div>
                            ...
                            <div className="rounded-lg border border-teal/20 bg-teal/5 p-3"><p className="text-sm font-semibold text-ink">SharePoint file destination</p><p className="mt-1 text-xs text-muted">Optional relative folder under this order’s SharePoint root. Future accepted files in this phase synchronize there. Leave blank for the phase default.</p><Input className="mt-3" label="Relative folder path" placeholder="Phase I/Docs" value={stage.sharePointDestination ?? ""} onChange={(event) => updateStage(index, { sharePointDestination: event.target.value || undefined })} help="Folders only; use letters, numbers, spaces, underscores, dashes, parentheses, periods, and forward slashes." /></div>
