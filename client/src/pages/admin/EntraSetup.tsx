...
export function AdminEntraSetupPage() {
  ...
  const [defaultRole, setDefaultRole] = useState<"customer" | "staff" | "admin">("staff");
  ...
  useEffect(() => {
    ...
    setDefaultRole((config.data.defaultRole === "admin" || config.data.defaultRole === "customer") ? config.data.defaultRole : "staff");
  ...
  return <>
    ...
    <div className="grid gap-6 lg:grid-cols-[0.8fr_1.2fr]">
      ...
      <Card><CardHeader title="Step 2 — Paste Entra values" description="The certificate is stored securely and is not displayed after saving." />
        <div className="mt-5 space-y-4"><FieldShell label="Configuration name" required><Input value={name} onChange={(event) => setName(event.target.value)} /></FieldShell><FieldShell label="Sign-on URL" required><Input value={entryPoint} onChange={(event) => setEntryPoint(event.target.value)} placeholder="https://login.microsoftonline.com/.../saml2" /></FieldShell><FieldShell label="Entity ID / Issuer" required><Input value={issuer} onChange={(event) => setIssuer(event.target.value)} placeholder="https://sts.windows.net/{tenant-id}/" /></FieldShell><FieldShell label="Base64 certificate" required help={config.data?.certificatePresent ? "A certificate is already saved; leave empty unless replacing it." : "Download Certificate (Base64) from Entra SAML Certificates."}><Textarea rows={7} value={certificate} onChange={(event) => setCertificate(event.target.value)} placeholder="-----BEGIN CERTIFICATE----- ..." /></FieldShell><div className="grid gap-4 sm:grid-cols-3"><FieldShell label="Provisioning"><Select value={autoProvision ? "yes" : "no"} onChange={(event) => setAutoProvision(event.target.value === "yes")}><option value="no">Only matched accounts</option><option value="yes">Provision automatically</option></Select></FieldShell><FieldShell label="New SAML user role" help="Applied only when automatic provisioning creates a new account."><Select value={defaultRole} onChange={(event) => setDefaultRole(event.target.value as "customer" | "staff" | "admin")}><option value="customer">Customer</option><option value="staff">Staff</option><option value="admin">Administrator</option></Select></FieldShell><FieldShell label="Activation"><Select value={enabled ? "enabled" : "disabled"} onChange={(event) => setEnabled(event.target.value === "enabled")}><option value="disabled">Save disabled until tested</option><option value="enabled">Enable Entra sign-in</option></Select></FieldShell></div><Alert tone="warning" title="Administrator provisioning">Choose Administrator only when the Entra assignment is restricted to trusted administrators. Existing local roles are not changed by this setting.</Alert><div className="flex justify-end"><Button busy={save.isPending} disabled={!name || !entryPoint || !issuer || (!certificate && !config.data?.certificatePresent)} leadingIcon={<ShieldCheck className="size-4" />} onClick={() => save.mutate({ name, entryPoint, issuer, idpCertificate: certificate || "preserved-certificate-placeholder", autoProvision, defaultRole, enabled })}>Save Entra configuration</Button></div></div>
      </Card>
    </div>
  </>;
}
