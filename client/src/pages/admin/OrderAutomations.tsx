...
type Rule = {
  ...
  triggerValue: string | null; actionType: "set_completion_percent" | "send_email" | "send_webhook"; completionPercent: number | null; emailTemplateKey: string | null; webhookEndpointId: number | null; isActive: boolean; sortOrder: number;
};
type AutomationAction = Rule["actionType"];
const actionLabel: Record<AutomationAction, string> = { set_completion_percent: "Set completion percentage", send_email: "Send email template", send_webhook: "Queue webhook delivery" };
...
export function AdminOrderAutomations() {
  ...
  const [actionType, setActionType] = useState<AutomationAction>("set_completion_percent");
  ...
  function openCreate() { setEditing(null); setName("Paid order progress"); setTriggerType("payment_status"); setTriggerValue("paid"); setPercent("2"); setActionType("set_completion_percent"); setEmailTemplateKey(""); setWebhookEndpointId(""); setActive(true); setCreating(true); }
  function openEdit(rule: Rule) { setEditing(rule); setName(rule.name); setTriggerType(rule.triggerType); setTriggerValue(rule.triggerValue ?? ""); setPercent(String(rule.completionPercent ?? 0)); setActionType(rule.actionType ?? "set_completion_percent"); setEmailTemplateKey(rule.emailTemplateKey ?? ""); setWebhookEndpointId(rule.webhookEndpointId ? String(rule.webhookEndpointId) : ""); setActive(rule.isActive); setCreating(true); }
  ...
  const columns: Column<Rule>[] = [
    ...
    { key: "action", header: "Action", cell: row => <span>{row.actionType === "set_completion_percent" ? <>Set completion to <strong>{row.completionPercent ?? 0}%</strong></> : row.actionType === "send_email" ? <>Send email <strong>{row.emailTemplateKey ?? "template"}</strong></> : <>Queue webhook <strong>#{row.webhookEndpointId ?? "—"}</strong></>}</span> },
  ...
  return <>
    <PageHeader title="Order automations" description="Automatically update completion, queue a customer email, or queue a signed webhook when payment, order status, intake, or phase events occur." actions={<Button variant="primary" leadingIcon={<Plus className="size-4" />} onClick={openCreate}>New automation</Button>} />
    <Card><CardHeader title="Lifecycle rules" description="Rules run server-side after the matching order event. Choose a completion update, an Email Template Center message, or an existing webhook endpoint." />
    ...
    <Modal open={creating} onClose={close} title={editing ? "Edit order automation" : "New order automation"} footer={<><Button variant="outline" onClick={close}>Cancel</Button><Button variant="primary" busy={save.isPending} disabled={!name.trim() || (actionType === "send_email" && !emailTemplateKey) || (actionType === "send_webhook" && !webhookEndpointId)} onClick={() => save.mutate({ id: editing?.id, name: name.trim(), triggerType, triggerValue: needsValue ? triggerValue || undefined : undefined, actionType, completionPercent: actionType === "set_completion_percent" ? Math.max(0, Math.min(100, Number(percent))) : undefined, emailTemplateKey: actionType === "send_email" ? emailTemplateKey : undefined, webhookEndpointId: actionType === "send_webhook" ? Number(webhookEndpointId) : undefined, isActive: active })}>Save automation</Button></>}>
      ...
      <Select label="Automation action" value={actionType} onChange={e => setActionType(e.target.value as AutomationAction)}><option value="set_completion_percent">Set completion percentage</option><option value="send_email">Send email template</option><option value="send_webhook">Queue webhook delivery</option></Select>
      {actionType === "set_completion_percent" ? <Input label="Set completion percentage" type="number" min="0" max="100" value={percent} onChange={e => setPercent(e.target.value)} required /> : null}
      {actionType === "send_email" ? <Select label="Email Template Center template" value={emailTemplateKey} onChange={e => setEmailTemplateKey(e.target.value)}><option value="">Select an email template</option>{(templates.data ?? []).map(template => <option key={template.templateKey} value={template.templateKey}>{template.name} — {template.templateKey}{template.enabled ? "" : " (disabled)"}</option>)}</Select> : null}
      {actionType === "send_webhook" ? <Select label="Webhook endpoint" value={webhookEndpointId} onChange={e => setWebhookEndpointId(e.target.value)}><option value="">Select an enabled webhook endpoint</option>{(endpoints.data ?? []).filter(endpoint => endpoint.enabled).map(endpoint => <option key={endpoint.id} value={String(endpoint.id)}>{endpoint.name} — {endpoint.url}</option>)}</Select> : null}
