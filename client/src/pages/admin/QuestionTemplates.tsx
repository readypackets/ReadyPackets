...
export function AdminQuestionTemplates() {
  ...
  const list = trpc.admin.questionTemplates.useQuery({ phase: "phase_1", includeInactive: true });
  const [editing, setEditing] = useState<Template | null>(null); const [open, setOpen] = useState(false);
  const [name, setName] = useState(""); const [question, setQuestion] = useState(""); const [required, setRequired] = useState(true); const [active, setActive] = useState(true);
  ...
  function create() { setEditing(null); setName(""); setQuestion(""); setRequired(true); setActive(true); setOpen(true); }
  function edit(t: Template) { setEditing(t); setName(t.name); setQuestion(t.question); setRequired(t.required); setActive(t.isActive); setOpen(true); }
  ...
  return <><PageHeader title="Phase 1 question templates" description="Maintain the reusable question bank that staff can add to any order during Phase 1." actions={<Button variant="primary" leadingIcon={<Plus className="size-4" />} onClick={create}>New template</Button>} />
    <Card><CardHeader title="Template bank" description="Templates are copied into an order so later edits never alter its historical questions." />{(list.data ?? []).length ? <DataTable caption="Phase 1 question templates" rows={(list.data ?? []) as Template[]} columns={columns} rowKey={row => row.id} /> : <EmptyState icon={ClipboardList} title="No templates yet" description="Create a reusable question to add it to Phase 1 orders." action={<Button variant="primary" onClick={create}>Create template</Button>} />}</Card>
    <Modal open={open} onClose={close} title={editing ? "Edit question template" : "New question template"} footer={<><Button variant="outline" onClick={close}>Cancel</Button><Button variant="primary" busy={save.isPending} disabled={name.trim().length < 2 || question.trim().length < 5} onClick={() => save.mutate({ id: editing?.id, name: name.trim(), question: question.trim(), phase: "phase_1", required, isActive: active, sortOrder: editing?.sortOrder ?? 0 })}>Save template</Button></>}><div className="space-y-4"><Input label="Template name" value={name} onChange={e => setName(e.target.value)} required /><Textarea label="Question" value={question} onChange={e => setQuestion(e.target.value)} rows={6} required /><Checkbox label="Require an answer before delivery" checked={required} onChange={e => setRequired(e.target.checked)} /><Checkbox label="Available to staff" checked={active} onChange={e => setActive(e.target.checked)} /></div></Modal>
  </>;
}
