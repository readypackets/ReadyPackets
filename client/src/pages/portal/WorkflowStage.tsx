  ...
  return <>
    ...
    <div className="grid gap-6 lg:grid-cols-2">
      ...
      <Card>
        ...
        {stage.uploadLimits?.audioTotalDurationSeconds !== undefined ? <p className="mt-3 text-xs text-muted">Audio time used: {formatDuration(audioUsedSeconds)} of {formatDuration(stage.uploadLimits.audioTotalDurationSeconds)}{totalAudioRemainingSeconds !== undefined ? ` · ${formatDuration(totalAudioRemainingSeconds)} remaining` : ""}.</p> : null}<ul className="mt-4 space-y-2">{audioFiles.length ? audioFiles.map((file) => <li key={file.id} className="rounded border border-line px-3 py-2 text-sm"><div className="flex flex-wrap items-center justify-between gap-3"><span className="min-w-0 truncate"><FileAudio className="mr-1 inline size-4" />{file.originalName}{file.durationSeconds ? <span className="text-xs text-muted"> · {formatDuration(file.durationSeconds)}</span> : null}</span><div className="flex items-center gap-2">{!file.uploadedByStaff && !phaseLocked ? <Button size="sm" variant="ghost" onClick={() => deleteFile.mutate({ fileId: file.id })}>Remove</Button> : file.uploadedByStaff ? <Badge tone="teal">Team recording</Badge> : <Badge tone="neutral">Locked</Badge>}</div></div><div className="mt-2"><AudioPlayback fileId={file.id} /></div></li>) : <li className="text-sm text-muted">No recordings for this phase yet.</li>}</ul>
      </Card>
