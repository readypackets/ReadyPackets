...
export function WorkflowStagePage() {
  ...
  return <>
    ...
    <Card className="mb-5"><CardHeader title="Order workflow" description="Complete the current step before the next step becomes available. Submitted steps stay available for review." /><ol className="mt-4 grid gap-2 md:grid-cols-2 xl:grid-cols-3">{(() => { const currentIndex = stages.findIndex((candidate) => candidate.key === currentStageKey); return stages.map((candidate, index) => { const lockedComplete = stageIsCompleted(candidate.key, completedKeys); const systemComplete = !lockedComplete && currentIndex > index; const completed = lockedComplete || systemComplete; const current = candidate.key === currentStageKey && !completed; const available = lockedComplete || current; return <li key={candidate.key} className={`flex items-center justify-between gap-3 rounded-lg border p-3 ${candidate.key === stage.key ? "border-teal bg-teal/5" : completed ? "border-success/30 bg-success/5" : "border-line bg-surface-soft"}`}><span className="min-w-0 text-sm"><span className={`mr-2 inline-flex size-6 items-center justify-center rounded-full text-xs font-bold ${completed ? "bg-success text-white" : current ? "bg-teal text-white" : "bg-surface-sunken text-muted"}`}>{completed ? <CheckCircle2 className="size-3.5" /> : candidate.order}</span>{candidate.label}</span>{systemComplete ? <Badge tone="success">Confirmed</Badge> : available ? <LinkButton size="sm" variant={current ? "primary" : "outline"} href={`/portal/orders/${orderId}/workflow/${candidate.key}`}>{lockedComplete ? "Review" : "Open"}</LinkButton> : <Badge tone="neutral">Upcoming</Badge>}</li>; }); })()}</ol></Card>
