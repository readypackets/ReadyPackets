...
struct OrdersView: View {
    ...
    var body: some View { NavigationStack { Group { if orders.isEmpty && error == nil { ContentUnavailableView("No orders", systemImage: "tray", description: Text("Your permitted ReadyPackets orders will appear here.")) } else { List { if let error { Text(error).foregroundStyle(Brand.danger) }; ForEach(orders) { order in NavigationLink(value: order) { OrderRow(order: order) } } }.navigationDestination(for: OrderSummary.self) { OrderDetailView(order: $0) } } }.navigationTitle("Orders").task { await load() }.refreshable { await load() } } }
