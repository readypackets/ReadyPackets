...
export const platformSetupRouter = router({
  ...
  saveAccess: adminProcedure.input(z.object({ ipPatterns: z.array(z.string().trim().min(3).max(64)).max(25), loginWhitelistEnabled: z.boolean(), loginWhitelistPublicIds: z.array(z.string().trim().regex(/^RP-U-[A-F0-9]{12}$/i)).max(200) })).mutation(async ({ ctx, input }) => {
